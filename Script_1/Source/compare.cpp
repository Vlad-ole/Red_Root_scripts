    ...
    for(int i = 0; i < run_list.size(); i++)
    {
        ...
        for (int ev = 0; ev < data->GetEntries(); ev++)
        {
            ...
            if(clusters.size() == 2)
            {
                ...
                bool cut_S1_total = clusters.at(0)->charge > 400 && clusters.at(0)->charge < 600;
                ...
                if (cut_f90 /*&& clusters.at(0)->rep == 1*/ && cut_S1_total /*&& cut_S2_total*/ && !cental_8_sipms_x )
                {
                    n_events_after_cuts++;
                    ...
                    h2_S2_total_tdrift_vec[i]->Fill(Tdrift, clusters.at(1)->charge);
                    h2_S1_total_tdrift_vec[i]->Fill(Tdrift, clusters.at(0)->charge);
