...
void sub_main1()
{
    ...
    path_file_out << "/home/vlad/Reports/S2_S1/run_" << run_number << ".txt";
    ofstream file_out(path_file_out.str().c_str());
    TString filename_in = path_file_out.str().c_str();
    ...
    for (int ev = 0; ev < data->GetEntries(); ev++)
    {
        ...
        if(nc == 2 /*&& clusters.at(0)->f90 > 0.3*/ && clusters.at(1)->f90 < 0.1 /* && clusters.at(0)->f90 > 0.3 && clusters.at(0)->f90 < 0.4*/)
        {
            ...
            file_out << Tdrift << "\t" << clusters.at(1)->charge/clusters.at(0)->charge << endl;
        }
